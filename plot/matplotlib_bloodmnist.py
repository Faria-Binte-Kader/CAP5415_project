...
# Load the JSON file
with open("bloodmnist_ViT_B_16_clip_metrics.json", "r") as file:
    data = json.load(file)
...
plt.title("BloodMNIST Accuracy Score Across Epochs")
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.legend()
plt.grid(True)
plt.savefig("bloodmnist_acc_metrics_plot.png")  # Save the plot
plt.close()
...
plt.title("BloodMNIST F1 Score Across Epochs")
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.legend()
plt.grid(True)
plt.savefig("bloodmnist_f1_metrics_plot.png")  # Save the plot
plt.close()
