...
# Load the JSON file
with open("organSmnist_ViT_B_16_clip_metrics.json", "r") as file:
    data = json.load(file)
...
plt.title("OrganSMNIST Accuracy Score Across Epochs")
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.legend()
plt.grid(True)
plt.savefig("organSmnist_acc_metrics_plot.png")  # Save the plot
plt.close()
...
plt.title("OrganSMNIST F1 Score Across Epochs")
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.legend()
plt.grid(True)
plt.savefig("organSmnist_f1_metrics_plot.png")  # Save the plot
plt.close()
