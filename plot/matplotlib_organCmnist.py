...
# Load the JSON file
with open("organCmnist_ViT_B_16_clip_metrics.json", "r") as file:
    data = json.load(file)
...
plt.title("OrganCMNIST Accuracy Score Across Epochs")
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.legend()
plt.grid(True)
plt.savefig("organCmnist_acc_metrics_plot.png")  # Save the plot
plt.close()
...
plt.title("OrganCMNIST F1 Score Across Epochs")
plt.xlabel("Epoch")
plt.ylabel("Score")
plt.legend()
plt.grid(True)
plt.savefig("organCmnist_f1_metrics_plot.png")  # Save the plot
plt.close()
